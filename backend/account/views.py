...
class LogoutView(APIView):
    ...
    def post(self, request):
        ...
        # 유효한 토큰만 블랙리스트에 추가
        if "username" not in token.payload:   # 또는 SIMPLE_JWT["USER_ID_CLAIM"]으로 동적으로 체크
...
class UserDetailView(APIView):
    ...
    def get(self, request, username): 
        user = get_object_or_404(get_user_model(), username=username)  # JWT 토큰으로 인증된 현재 유저 정보 가져오기
