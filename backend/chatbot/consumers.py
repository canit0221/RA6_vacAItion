...
class ChatConsumer(AsyncWebsocketConsumer):
        ...
        try:
            ...
            if date_param:
                try:
                    ...
                    try:
                        ...
                        print(f"✅ URL에서 날짜 파라미터 성공적으로 추출: {date_param} (파싱됨: {parsed_date})")
                        # 나중에 사용하기 위해 인스턴스 변수로 저장
                        self.session_date = parsed_date
                    except ValueError:
                        print(f"❌ 날짜 형식 오류: {date_param}은 올바른 날짜 형식(YYYY-MM-DD)이 아닙니다")
            ...
            try:
                ...
                if date_param:
                    ...
                    print(f"✅ 세션 {self.room_name}에 URL 파라미터 저장 완료: {url_params}")
            except Exception as e:
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            print(f"추출된 메시지: '{message}', 세션 ID: {session_id or self.room_name}")
            ...
            # 메시지 저장
            session, is_new = await self.save_message_and_get_response(message, session_id)
        ...
        try:
            ...
            try:
                ...
                try:
                    if hasattr(session, 'date') and session.date:
                        session_date = await database_sync_to_async(lambda: session.date)()
                        print(f"✅ 세션 객체에서 날짜 정보 가져옴: {session_date}")
                except Exception as e:
                    print(f"❌ 세션 date 필드 접근 중 오류: {e}")
                
                # 2. self.session_date에서 가져오기 (connect 메서드에서 설정된 값)
                if not session_date and hasattr(self, 'session_date') and self.session_date:
                ...
                if not session_date:
                    try:
                        session_url_params = await database_sync_to_async(
                            lambda: session.url_params if hasattr(session, 'url_params') and session.url_params else {}
                        )()
                        
                        if isinstance(session_url_params, dict) and 'date' in session_url_params:
                            from datetime import datetime
                            date_str = session_url_params.get('date')
                            try:
                                session_date = datetime.strptime(date_str, "%Y-%m-%d").date()
                                print(f"✅ URL 파라미터에서 날짜 정보 가져옴: {session_date}")
                            except Exception as e:
                ...
                if not session_date:
                    ...
                    print(f"⚠️ 날짜 정보를 찾을 수 없어 현재 날짜를 사용합니다: {session_date}")
                ...
                # 일정 정보 조회 준비
                print(f"📅 일정 조회 - 날짜: {session_date}, 사용자: {self.user.username}")
                ...
                try:
                    ...
                    # 지정된 날짜의 일정 조회
                    user_schedule = await database_sync_to_async(lambda: Schedule.objects.filter(
                        user=self.user, 
                        date=session_date
                    ).first())()
                    ...
                    if user_schedule:
                        ...
                        if hasattr(user_schedule, 'companion') and user_schedule.companion:
                            ...
                            print(f"✅ 일정에서 동행자 정보 가져옴: '{schedule_companion}'")
                        
                        print(f"✅ 사용자 {self.user.username}의 {session_date} 일정 정보를 성공적으로 가져왔습니다.")
                    else:
                        print(f"⚠️ 사용자 {self.user.username}의 {session_date} 일정이 없습니다.")
                except Exception as e:
            ...
            try:
                ...
                if schedule_place:
                    ...
                    state["place"] = schedule_place  # 호환성을 위해 두 가지 키 모두 설정
                    print(f"✅ 그래프에 일정 장소 정보 전달: '{schedule_place}'")
                
                if schedule_companion:
                    state["companion"] = schedule_companion
                    state["with_who"] = schedule_companion  # 호환성을 위해 두 가지 키 모두 설정
    ...
    def save_message_and_get_response(self, message, session_id):
        ...
        try:
            ...
            session = ChatSession.objects.create(
                user=self.user, 
                title=f"오류 복구 채팅 {self.user.username}"
            )
            ChatMessage.objects.create(session=session, content=message, is_bot=False)
            return session, True
        ...
        try:
            ...
            if session and date_param:
                try:
                    ...
                    session.date = session_date
                    session.save()
                    print(f"✅ 기존 세션 {session_id}의 날짜를 {session_date}로 업데이트")
                except Exception as e:
            ...
            if session:
                ...
                if date_param:
                    try:
                        ...
                        session.date = session_date
                        session.save()
                        print(f"✅ session_id로 찾은 세션 {session_id}의 날짜를 {session_date}로 업데이트")
                    except Exception as e:
    ...
    async def process_message(self, user_message, session_id):
        ...
        try:
            ...
            session, created = await self.save_message_and_get_response(user_message, session_id)
            ...
            print(f"=== 클라이언트로 메시지 전송: 타입=사용자, 스트리밍=False, 내용={user_message[:50] if len(user_message) > 50 else user_message}... ===")
